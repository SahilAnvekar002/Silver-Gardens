...
function AcceptAppointmentModal({ setIsOpen, currentId, currentStatus, currentEmail }: PropsType) {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        setIsOpen(false);
        const res2 = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/send-email`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
                ...
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all lg:my-8 lg:w-full lg:max-w-lg">
                        <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 dark:bg-slate-800">
                            <div className="sm:flex sm:items-start">
                                <div className="mt-3 sm:ml-4 sm:mt-0 sm:text-left w-full">
                                    <h3 className="mb-3 text-base font-semibold leading-6 text-gray-900 dark:text-white" id="modal-title">Appointment Details</h3>
                                            ...
                                            <form ref={ref} onSubmit={(e) => handleSubmit(e)}>
                                                <div className="mt-2 grid grid-cols-1 gap-x-0 gap-y-6 sm:grid-cols-6">
                                                    <div className="sm:col-span-full">
                                                        <label htmlFor="time" className="dark:text-gray-500 block text-sm font-medium leading-6 text-gray-900">Appointment Time</label>
                                                        <div className="mt-2">
                                                            <input type="datetime-local" name="time" id="time" className="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 outline-none px-3 dark:focus:ring-white" value={time} onChange={(e) => handleChange(e.target.value)} required  min={new Date().toISOString().slice(0, 16)}/>
                                                        </div>
                                                    </div>
                        ...
                        <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row sm:px-6 dark:bg-slate-800">
                            <button type="button" className="inline-flex w-full justify-center rounded-md bg-blue-700 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 sm:ml-3 sm:w-auto dark:bg-transparent dark:hover:bg-slate-900 dark:border-2 dark:border-gray-700" onClick={() => handleClick()}>Accept Appointment</button>
